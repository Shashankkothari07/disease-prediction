...
filepath = Path(__file__).parent / "Testing.csv"


tr=pd.read_csv(filepath)
...
# TRAINING DATA
filepath2 = Path(__file__).parent / "Training.csv"
...
def message():
    if (Symptom1.get() == "None"  or Symptom2.get() == "None" ):
        messagebox.showinfo("OPPS!!", "At least enter first 2 SYMPTOMS PLEASE")
    else :
        NaiveBayes()
...
lr = Button(root, text="Naive-Byes",height=2, width=10, command=message)
...
t5 = Text(root, height=2, width=20)
t5.config(font=("Elephant", 20))
t5.grid(row=17, column=1 ,padx=60)
root.mainloop()